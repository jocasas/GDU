...
describe('User API integration tests', () => {
    beforeEach(() => {
        users.length = 0; // Resetear in-memory DB
    });
    ...
    it('should delete a user if not birthday', async () => {
        ...
        expect(res.body).toBeDefined(); // o si devuelve el objeto eliminado:
    ...
    it('should get all users', async () => {
        // Puedes preparar datos si quieres asegurarte de algo específico
        users.length = 0;
        users.push({
            rut: '12345678-9',
            nombre: 'Juan',
